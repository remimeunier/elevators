defmodule Elevator.ElevatorRegistryTest do
  ...
  test "the registry should keep the elevator associated to a name", %{registry: registry} do
    ...
    assert Elevator.Elevator.get(elevator1) == %{current_position: 0,
                                               destination: [],
                                               iddle_destination: [],
    ...
    assert Elevator.Elevator.get(elevator2) == %{current_position: 0,
                                                destination: [],
                                                iddle_destination: [],
                                                state: Const.state_idle,
    ...
    assert Elevator.Elevator.get(elevator3) == %{current_position: 6,
                                                destination: [],
                                                iddle_destination: [],
                                                state: Const.state_idle,
                                                display_name: 'elevator_3'}
    assert Elevator.ElevatorRegistry.index(registry) == %{'elevator_1' => elevator1,
